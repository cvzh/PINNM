function pos_arr0 = match_block(image_patch,X,par)

par         = parasetting(par);
m           = par.every_nblk; 
all_nblk    = par.nblk;  % Number of Similar Patches
win         = par.win;   % Patch size
step        = par.step;  % Stride
N           = par.N-win+1;
M           = par.M-win+1;
r           = 1:step:N;
r           = [r r(end)+1:N];
   ...
   for i = 1 : N1
       for j = 1 : M1
        ...
        if M*N*(n-1) < off && off <= M*N*n
            ii      =   idx==off;
            idx(ii) =   [];
            dis     =   (mX(idx, :) + mx(off - (n-1) * N * M, :) - X(idx, :)*x(off - (n-1) * N * M, :)'); 
            [val,ind]               =   sort(dis);
